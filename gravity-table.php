...
$updater->authorize( 'dccf46075a714bb6a792efc6c516e2cafaff5535' ); // Your auth code goes here for private repos
...
include_once plugin_dir_path( __FILE__ ) .'include/class-gtv-update.php'; 
...
function url_payment()
{
 ...
 $get_amount = gform_get_meta( $_POST['get_entry'], 'payment_amount' );
	//print_r($form);
	// print_r( GFAPI::get_forms($_POST['form_id'], "title"));
	// print_r(RGFormsModel::get_forms($_POST['form_id']));
	$submission_data =Array(
...
foreach($gform_product_info__['products'] as $key => $value)
{
 ...
 //array_push($submission_data['line_items'], Array ( 'id' => 96,'name' => 'DA 10+ Websites', 'description'=> "",'quantity'=>1,'unit_price'=> 40,  'options' => ""));
 //print_r($gform_product_info__);

 //gf_apply_filters( 'gform_paypal_request', $data->form_id, "http://onlinemarketing.guru/booking/", $form, GFAPI::get_entry( $data->id ), paypal_get_feeds( $data->form_id),$submission_data ) ;
 
 add_filter( 'gform_paypal_request', 'update_url', 10, 5 );
echo gf_apply_filters( 'gform_paypal_request', $_POST['form_id'], "http://onlinemarketing.guru/booking/asdfas", $form ,GFAPI::get_entry(  $_POST['get_entry'] ), paypal_get_feeds( $_POST['form_id'] ),$submission_data ) ;
	// echo $_POST['form_id'] . " " . $_POST['url'] . " " . $_POST['get_entry'] ;
	//print_r( );
	//header("Location:".gf_apply_filters( 'gform_paypal_request', 1, $_POST['url'] , $form, GFAPI::get_entry(  5810 ), paypal_get_feeds(1),$submission_data ));
}
