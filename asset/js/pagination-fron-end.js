   ...
   jQuery.ajax({
		...
		success : function( response )
		{
		 ...
		 var pricecount = 0;
				 ...
				 {  
					...
					var classname = "row";
						if( id.match(/\((.*)\)/) && regex.test(id) == true)
						{
						 ...
						 var row = jQuery('<li id="'+id.replace(/["'()]/g,"").replace(/[^a-z0-9\s]/gi, '').replace(/[_\s]/g, ' ').replace(/ /g,'')+'" class="'+classname+'"><div class="entry-view-field-name">'+k.replace(/_/g, ' ')+'</div><div class="entry-view-field-value">'+v+'</div></li>');
						  table.append(row);
					} 
				 }
				});
			...
			jQuery('.modal-content #details').html(table);
